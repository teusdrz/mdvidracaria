...
export default function WhyChooseUs() {
  ...
  const onEnter = useCallback((el: HTMLDivElement) => {
    gsap.to(el, { y: -7, boxShadow: "0 20px 52px rgba(28,69,135,0.13)", borderColor: "rgba(28,69,135,0.30)", duration: 0.32, ease: "power2.out" });
    gsap.to(el.querySelector(".wcu-icon-bg"), { backgroundColor: "var(--color-primary)", duration: 0.28, ease: "power2.out" });
    gsap.to(el.querySelector(".wcu-icon-svg"), { color: "#ffffff", duration: 0.28, ease: "power2.out" });
    gsap.to(el.querySelector(".wcu-title"), { color: "var(--color-primary)", duration: 0.25, ease: "power2.out" });
  }, []);

  const onLeave = useCallback((el: HTMLDivElement) => {
    gsap.to(el, { y: 0, boxShadow: "0 1px 8px rgba(0,0,0,0.06)", borderColor: "rgba(0,0,0,0.09)", duration: 0.38, ease: "power2.inOut" });
    gsap.to(el.querySelector(".wcu-icon-bg"), { backgroundColor: "rgba(28,69,135,0.07)", duration: 0.32, ease: "power2.inOut" });
    gsap.to(el.querySelector(".wcu-icon-svg"), { color: "var(--color-primary)", duration: 0.32, ease: "power2.inOut" });
    gsap.to(el.querySelector(".wcu-title"), { color: "#1a1a1a", duration: 0.28, ease: "power2.inOut" });
  }, []);
          ...
          {REASONS.map(({ icon: Icon, title, description, backStat, backLabel, backQuote }) => (
            ...
            >
              ...
              >
                ...
                >
                  <div
                    className="wcu-icon-bg w-12 h-12 rounded-xl flex items-center justify-center"
                  ...
                  <h3
                    className="wcu-title mt-8 text-[15px] font-semibold leading-snug"
                  ...
                  <p
                    className="mt-4 mb-auto text-[13px] leading-[1.8] text-neutral-400"
